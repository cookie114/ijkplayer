    ...
    do {
        ...
        if (!d->packet_pending || d->queue->serial != d->pkt_serial) {
            ...
            do {
				...
				//ALOGI("Buffer Monitor:Type[%d]=nb_packets=%d=Size=%d================================\n",d->avctx->codec_type,d->queue->nb_packets,d->queue->size);
...
static int read_thread(void *arg)
{
    ...
    for (;;) {
		if(time(NULL)-print_tick >=1)
		{
			print_tick = time(NULL);
			ALOGI("Buffer Monitor:Time[%03d]:Audio_Size[%d],Video_Size[%d]=Total:%d=================================\n",
				print_tick-start_tick,is->audioq.size,is->videoq.size,is->audioq.size+is->videoq.size);
		}	
